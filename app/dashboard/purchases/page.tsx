...
export default function PurchasesPage() {
  ...
  return <div className="mx-auto flex min-h-[calc(100vh-88px)] max-w-[1680px] flex-col gap-3 pb-28 lg:pb-3">
    <section className="shrink-0 rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex min-h-12 flex-wrap items-center gap-2 px-3 py-2"><div className="flex items-center gap-2 pr-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-600 text-white"><ShoppingBag className="h-4 w-4" /></span><div className="leading-tight"><div className="text-sm font-black text-slate-900">Purchase Invoice</div><div className="text-[10px] text-slate-500">Stock Inward</div></div></div><div className="hidden h-7 w-px bg-slate-200 sm:block" /><span className="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-wide text-emerald-700">Live POS</span><span className="hidden text-xs text-slate-400 lg:inline">•</span><span className="hidden text-xs text-slate-500 lg:inline">New Purchase</span><div className="ml-auto flex items-center gap-2"><span className="hidden rounded-lg bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-500 md:inline">Press <b>/</b> for products</span><button type="button" onClick={() => void load()} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 hover:bg-slate-50"><RefreshCw className="h-3.5 w-3.5" />Refresh</button></div></div></section>

    <div className="mb-0 flex items-center justify-between gap-2 rounded-2xl border border-violet-100 bg-violet-50/70 px-3 py-2 sm:px-4"><div className="flex min-w-0 items-center gap-2"><CalendarDays className="h-4 w-4 shrink-0 text-violet-600" /><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.16em] text-violet-700">Invoice Date</p><p className="hidden text-[10px] text-violet-600 sm:block">Date for this new purchase</p></div></div><input type="date" value={invoiceDate} max={today()} onChange={e => changeInvoiceDate(e.target.value)} className="h-9 rounded-xl border border-violet-200 bg-white px-2.5 text-sm font-bold text-slate-800 outline-none focus:border-violet-500" /></div>
    ...
    <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_250px]">
      <main className="flex min-h-0 min-w-0 flex-col gap-3">
        <section className="shrink-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="grid gap-2 lg:grid-cols-[minmax(250px,.7fr)_minmax(0,1.6fr)]">
          ...
          <div className="relative"><label className="mb-1 block text-[10px] font-black uppercase tracking-wider text-slate-500">Product / Barcode</label><div className={`flex h-11 items-center gap-2 rounded-xl border px-3 ${showProductResults ? 'border-violet-500 ring-2 ring-violet-100' : 'border-slate-200 bg-slate-50'}`}><Search className="h-4 w-4 shrink-0 text-slate-400" /><input ref={productInput} value={productSearch} onChange={e => { setProductSearch(e.target.value); setShowProductResults(true) }} onFocus={() => setShowProductResults(true)} placeholder="Search product, SKU or scan barcode…" className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none" />{productLoading && <Loader2 className="h-4 w-4 animate-spin text-violet-600" />}<span className="rounded-md bg-white px-1.5 py-1 text-[10px] font-bold text-slate-400 shadow-sm">/</span></div>{showProductResults && <div className="absolute left-0 right-0 top-[62px] z-40 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl"><div className="max-h-[52vh] overflow-y-auto">{products.slice(0, 12).map(p => <button type="button" key={p.id} onMouseDown={e => e.preventDefault()} onClick={() => addProduct(p)} className="flex w-full items-center gap-2 border-b border-slate-100 px-3 py-2.5 text-left hover:bg-violet-50"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[10px] font-black text-slate-500">{p.name.slice(0, 2).toUpperCase()}</span><span className="min-w-0 flex-1"><b className="block truncate text-xs text-slate-900">{p.name}</b><small className="block truncate text-[10px] text-slate-500">{p.sku} · Stock {p.current_stock}</small></span><span className="text-xs font-black">{money(p.purchase_price)}</span><Plus className="h-4 w-4 text-violet-600" /></button>)}{!products.length && <div className="p-6 text-center text-xs text-slate-500">{productSearch ? 'No matching product' : 'Type to search products'}</div>}</div></div>}</div>
        </div></section>

        <section className="flex min-h-[600px] flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-slate-50/80 px-3 py-2.5"><div className="flex items-center gap-2"><h2 className="text-sm font-black text-slate-900">Invoice Items</h2><span className="rounded-md bg-violet-100 px-2 py-0.5 text-[10px] font-black text-violet-700">{itemCount} Qty</span>{selectedSupplier && <span className="hidden text-[11px] text-slate-500 sm:inline">• {selectedSupplier.name}</span>}</div><button type="button" onClick={() => setLines([])} disabled={!lines.length || saving} className="inline-flex h-8 items-center gap-1 rounded-lg border border-rose-200 bg-white px-2.5 text-[11px] font-bold text-rose-600 disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" />Clear</button></div><div className="min-h-0 flex-1 overflow-auto">{lines.length ? <table className="w-full min-w-[720px] text-sm"><thead className="sticky top-0 z-10 bg-white text-[10px] font-black uppercase tracking-wide text-slate-500 shadow-[0_1px_0_#e2e8f0]"><tr><th className="w-1/2 px-3 py-2 text-left">Product</th><th className="px-2 py-2 text-center">Stock</th><th className="px-2 py-2 text-center">Qty</th><th className="px-2 py-2 text-right">Rate</th><th className="px-3 py-2 text-right">Amount</th><th className="w-8" /></tr></thead><tbody className="divide-y divide-slate-100">{lines.map(line => { const product = products.find(p => p.id === line.product_id); return <tr key={line.product_id} className="hover:bg-violet-50/40"><td className="px-3 py-2"><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-[10px] font-black text-slate-500">{product?.name?.slice(0, 2).toUpperCase() || 'PR'}</div><div className="min-w-0"><div className="truncate text-xs font-bold text-slate-900">{product?.name || 'Product'}</div><div className="truncate text-[10px] text-slate-500">{product?.sku}</div></div></div></td><td className="px-2 py-2 text-center text-[11px] font-semibold text-slate-500">{product?.current_stock ?? 0}</td><td className="px-2 py-2"><div className="mx-auto flex h-8 w-[92px] items-center justify-between rounded-lg border border-slate-200 bg-white"><button type="button" onClick={() => updateQty(line.product_id, line.quantity - 1)} className="flex h-full w-7 items-center justify-center text-slate-500 hover:bg-slate-50"><Minus className="h-3.5 w-3.5" /></button><input type="number" min="1" value={line.quantity} onChange={e => updateQty(line.product_id, Number(e.target.value))} className="w-10 bg-transparent text-center text-xs font-black outline-none" /><button type="button" onClick={() => updateQty(line.product_id, line.quantity + 1)} className="flex h-full w-7 items-center justify-center text-violet-600 hover:bg-violet-50"><Plus className="h-3.5 w-3.5" /></button></div></td><td className="px-2 py-2 text-right"><input type="number" min="0" step="0.01" value={line.unit_price} onChange={e => updatePrice(line.product_id, e.target.value)} className="h-8 w-24 rounded-lg border border-slate-200 bg-white px-2 text-right text-xs font-bold outline-none focus:border-violet-500" /></td><td className="px-3 py-2 text-right text-xs font-black text-slate-900">{money(line.quantity * line.unit_price)}</td><td className="px-2"><button type="button" onClick={() => updateQty(line.product_id, 0)} className="rounded-md p-1 text-slate-300 hover:bg-rose-50 hover:text-rose-600"><X className="h-4 w-4" /></button></td></tr> })}</tbody></table> : <div className="flex h-full min-h-[540px] flex-col items-center justify-center px-6 text-center"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-50 text-violet-600"><ShoppingBag className="h-7 w-7" /></div><h3 className="mt-3 text-base font-black text-slate-800">Start your purchase</h3><p className="mt-1 max-w-sm text-xs text-slate-500">Search a product above or scan its barcode. Added items will appear here with quantity, purchase rate and amount.</p><button type="button" onClick={() => productInput.current?.focus()} className="mt-4 h-9 rounded-lg bg-violet-600 px-4 text-xs font-bold text-white hover:bg-violet-700">Search Products</button></div>}</div><div className="flex shrink-0 items-center justify-between border-t border-slate-200 bg-slate-50 px-3 py-2 text-xs"><span className="text-slate-500">{lines.length} products · {itemCount} units</span><span className="font-black text-slate-900">Net Purchase <b className="ml-2 text-sm">{money(total)}</b></span></div></section>
      </main>
