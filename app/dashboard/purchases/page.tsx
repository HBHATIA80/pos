      ...
      <section className="shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex min-h-12 flex-wrap items-center gap-2 px-3 py-2">
          <div className="flex items-center gap-3 pr-2">
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-700 text-white shadow-sm"><ShoppingBag className="h-6 w-6 stroke-[2.5]" /></span>
            <div className="leading-tight"><div className="text-base font-black text-black">Purchase Invoice</div><div className="text-[11px] font-bold text-slate-700">Stock Inward</div></div>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button type="button" onClick={() => setShowVouchers(true)} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-300 bg-slate-50 px-3 text-xs font-black text-black hover:bg-slate-100"><History className="h-4 w-4" />Latest 20 Vouchers</button>
            ...
            <button type="button" onClick={() => void load()} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 text-xs font-black text-black hover:bg-slate-50"><RefreshCw className="h-4 w-4" />Refresh</button>
          </div>
        </div>
      </section>
            ...
            <div className="grid gap-3 lg:grid-cols-[minmax(250px,.7fr)_minmax(0,1.6fr)]">
              <div className="relative">
                ...
                <div className={`flex h-11 items-center gap-2 rounded-xl border px-3 ${showSupplierResults ? 'border-slate-600 bg-white ring-2 ring-slate-200' : 'border-slate-300 bg-slate-50'}`}>
                  ...
                  <input ref={supplierInput} value={selectedSupplier?.name || supplierSearch} onChange={event => { setSupplier(''); setSupplierSearch(event.target.value); setShowSupplierResults(true) }} onFocus={() => setShowSupplierResults(true)} placeholder="Walk-in supplier / name / mobile" className="min-w-0 flex-1 bg-transparent text-sm font-bold text-black outline-none placeholder:text-slate-600" />
              ...
              <div className="relative">
                ...
                {showProductResults && <div className="absolute left-0 right-0 top-[64px] z-40 overflow-hidden rounded-xl border border-slate-300 bg-white shadow-2xl"><div className="max-h-[55vh] overflow-y-auto">{products.slice(0, 12).map(product => <button type="button" key={product.id} onMouseDown={event => event.preventDefault()} onClick={() => addProduct(product)} className="w-full border-b border-slate-200 px-3 py-3 text-left hover:bg-slate-100"><div className="flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-700"><ShoppingBag className="h-5 w-5" /></div><div className="min-w-0 flex-1"><b className="block truncate text-sm font-black text-black">{product.name}</b><span className="mt-0.5 block truncate text-[11px] font-black text-black">SKU: {product.sku} · <span className="text-emerald-800">Qty Available: {product.current_stock}</span></span><div className="mt-2 flex flex-wrap gap-1.5"><span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-950 ring-1 ring-slate-200"><span className="text-slate-700">Selling</span>{money(Number(product.sale_price))}</span><span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-950 ring-1 ring-slate-200"><span className="text-slate-700">Purchase</span>{money(Number(product.purchase_price))}</span><span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-950 ring-1 ring-slate-200"><span className="text-slate-700">Last Purchase</span>{money(Number(product.purchase_price))}</span></div></div><Plus className="h-5 w-5 shrink-0 text-slate-700" /></div></button>)}{!products.length && <div className="p-6 text-center text-sm font-bold text-black">{productSearch ? 'No matching product' : 'Type to search products'}</div>}</div></div>}
              </div>
            </div>
          </section>
        ...
        <aside className="max-xl:fixed max-xl:bottom-16 max-xl:left-2 max-xl:right-2 max-xl:z-40 xl:sticky xl:top-3 xl:self-start">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg max-xl:shadow-2xl">
            ...
            <div className="hidden space-y-2 px-3 py-3 text-sm xl:block">
              ...
              <div className="rounded-xl bg-slate-700 px-3 py-3">
                <div className="flex items-end justify-between gap-3"><span className="font-black !text-white">Net Purchase</span><span className="text-xl font-black !text-white">{money(total)}</span></div>
              </div>
            </div>
            <div className="border-t border-slate-200 p-2.5"><button type="button" disabled={saving || !lines.length} onClick={() => void save()} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-700 text-sm font-black text-white shadow-sm hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40">{saving ? <><Loader2 className="h-4 w-4 animate-spin" />Saving…</> : <><Check className="h-4 w-4" />Complete Purchase</>}</button></div>
          </section>
        </aside>
      </div>
