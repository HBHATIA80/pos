...
export default function POSShell({ children, profile, businessName }: Props) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className={`min-h-screen transition-all duration-200 ${collapsed ? 'lg:pl-[76px]' : 'lg:pl-64'}`}>
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:px-8"><div className="flex min-w-0 items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="rounded-xl p-2 text-slate-600 hover:bg-slate-100 lg:hidden" aria-label="Open menu"><Menu className="h-5 w-5" /></button><div className="min-w-0"><p className="truncate text-sm font-black text-slate-950">{businessName}</p><p className="hidden text-xs text-slate-500 sm:block">{isCustomer ? 'BIZBook Customer Portal' : 'BIZBook POS Workspace'}</p></div></div><div className="flex items-center gap-2 sm:gap-3"><a href={MARKETPLACE} className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 text-xs font-black text-white shadow-sm transition hover:bg-indigo-700 sm:px-4 sm:text-sm ${pathname === MARKETPLACE ? 'ring-4 ring-indigo-100' : ''}`}><Store className="h-4 w-4" /><span>Marketplace</span></a><div className="hidden text-right sm:block"><p className="max-w-[180px] truncate text-sm font-medium">{profile.fullName}</p><p className="text-xs capitalize text-slate-500">{isCustomer ? 'Customer' : profile.role}</p></div><LogoutButton /></div></div></header>
      <main className="px-3 pb-24 pt-4 sm:px-6 sm:pt-6 lg:px-8 lg:pb-8">{children}</main>
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 shadow-[0_-8px_30px_rgba(15,23,42,0.06)] backdrop-blur"><div className={`mx-auto grid max-w-lg gap-1 ${isCustomer ? 'grid-cols-3' : 'grid-cols-4'}`}>{isCustomer ? <><a href="/dashboard/orders" className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-2 ${pathname === '/dashboard/orders' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`}><ShoppingCart className="h-5 w-5" /><span className="text-[10px] font-bold">Shop</span></a><a href={MARKETPLACE} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-2 ${pathname === MARKETPLACE ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`}><Store className="h-5 w-5" /><span className="text-[10px] font-bold">Market</span></a><button type="button" onClick={() => setMobileOpen(true)} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-2 text-slate-500"><Menu className="h-5 w-5" /><span className="text-[10px] font-bold">Menu</span></button></> : <><Link href="/dashboard" className="flex flex-col items-center gap-1 rounded-xl px-2 py-1.5"><LayoutDashboard className="h-5 w-5" /><span className="text-[10px] font-medium">Home</span></Link><Link href="/dashboard/inventory" className={`flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 ${pathname === '/dashboard/inventory' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`}><Package className="h-5 w-5" /><span className="text-[10px] font-medium">Inventory</span></Link><a href={MARKETPLACE} className={`flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 ${pathname === MARKETPLACE ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`}><Store className="h-5 w-5" /><span className="text-[10px] font-medium">Market</span></a><Link href="/dashboard/sales" className="flex flex-col items-center gap-1 rounded-xl px-2 py-1.5"><ShoppingCart className="h-5 w-5" /><span className="text-[10px] font-medium">Sales</span></Link></>}</div></nav>
    </div>
  </div>
}
