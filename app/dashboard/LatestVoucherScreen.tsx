...
const partyName = (voucher: Voucher, fallback: string) =>
  Array.isArray(voucher.party)
    ? voucher.party[0]?.name || fallback
    : voucher.party?.name || fallback

const partyPhone = (voucher: Voucher) =>
  Array.isArray(voucher.party) ? voucher.party[0]?.phone : voucher.party?.phone
...
export default function LatestVoucherScreen() {
  ...
  async function load(kind: 'sales' | 'purchases') {
    ...
    try {
      const response = await fetch(
        kind === 'sales' ? '/api/sales' : '/api/purchases',
        { cache: 'no-store' },
      )
  ...
  async function selectVoucher(voucher: Voucher) {
    ...
    try {
      const response = await fetch(`/api/sales/${voucher.id}/payment`, {
        cache: 'no-store',
      })
      const body = await response.json().catch(() => ({}))
  ...
  useEffect(() => {
    const handler = (event: MouseEvent) => {
      ...
      if (
        !path.endsWith('/dashboard/sales') &&
        !path.endsWith('/dashboard/purchases')
      ) {
        return
      }
  ...
  const filtered = vouchers.filter((voucher) => {
    const haystack = `${voucher.invoice_no} ${partyName(
      voucher,
      type === 'sales' ? 'Walk-in Customer' : 'Walk-in / Other',
    )} ${partyPhone(voucher) || ''}`.toLowerCase()
  ...
  const allDraftSelected =
    ...
    draftVouchers.every((voucher) => selectedIds.includes(voucher.id))
  ...
  async function deleteSelected() {
    ...
    if (
      !window.confirm(
        `Delete ${selectedIds.length} selected draft purchase${
  ...
  return (
    <div
      className={`${styles.modalRoot} fixed inset-0 z-[120] bg-slate-950/60 p-0 sm:p-4`}
      role="dialog"
      aria-modal="true"
      aria-labelledby="latest-vouchers-title"
    >
      <div className="mx-auto flex h-full max-h-[92vh] w-full flex-col overflow-hidden rounded-none bg-white shadow-2xl sm:my-2 sm:rounded-3xl">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1
                  id="latest-vouchers-title"
                  className="text-xl font-black text-black"
                >
                  Latest 20 Purchase Vouchers
                </h1>
              </div>
              <p className="mt-1 text-xs font-semibold text-black">
                Review purchase history and safely remove drafts.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <div className="relative min-w-0 flex-1 sm:min-w-[260px] sm:max-w-md">
              ...
              <input
                ...
                placeholder="Search invoice, party or mobile..."
                aria-label="Search vouchers"
                className="h-10 w-full rounded-xl border border-slate-300 bg-white pl-9 pr-3 text-sm font-semibold text-black outline-none focus:border-emerald-600"
              />
            </div>
            ...
            {type === 'purchases' && (
              <>
                  ...
                  <input
                    ...
                    onChange={() =>
                      setSelectedIds(
                        allDraftSelected
                          ? []
                          : draftVouchers.map((voucher) => voucher.id),
                      )
                    }
                    className="h-4 w-4"
                  />
                ...
                <button
                  ...
                  className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white disabled:opacity-40"
                >
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[350px_minmax(0,1fr)]">
          <section
            className="min-h-0 overflow-x-hidden overflow-y-auto border-b border-slate-200 lg:border-b-0 lg:border-r"
            aria-label="Latest vouchers"
          >
            <div className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50 px-4 py-3 text-[11px] font-black uppercase tracking-wider text-black">
              Select <span className="mx-2">·</span> Voucher{' '}
              <span className="mx-2">·</span> Supplier
            </div>
              ...
              <div className="mx-auto max-w-4xl space-y-4">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black uppercase ${
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-3">
                    ...
                    <Info
                      icon={<Phone />}
                      label="Mobile"
                      value={partyPhone(selected) || 'Not provided'}
                    />
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  ...
                  {selected.items?.length ? (
                                ...
                                <td className="px-5 py-3 font-black text-black">
                                  ...
                                  {hasDiscount ? (
                                    <div className="text-[10px] font-bold text-slate-500">
                                      Discount: {money(Number(item.discount_amount))}
                                    </div>
                                  ) : null}
                                </td>
                    ...
                    <div className="p-8 text-center text-sm font-bold text-slate-600">
                      No item lines are available for this voucher.
                    </div>
                  )}
                </div>
                      ...
                      <Info
                        ...
                        value={
                          selected.completed_at
                            ? new Date(selected.completed_at).toLocaleString('en-IN')
                            : 'Not completed'
                        }
                      />
                      <div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-4 space-y-3 text-sm">
                      ...
                      <div className="mt-4 rounded-xl bg-amber-50 px-4 py-4">
                        <Row label="Grand Total" value={money(selected.grand_total)} strong />
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    {detailLoading ? (
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        <Info
                          ...
                          value={payment.payment_status || 'Not available'}
                        />
                        <Info
                          icon={<CheckCircle2 />}
                          label="Amount paid"
                        ...
                        <Info
                          icon={<FileText />}
                          label="Balance"
                          value={money(payment.balance_amount)}
                        />
                      </div>
                    ) : (
                      <p className="mt-3 text-xs font-semibold text-slate-600">
                        Payment details are not available for this voucher.
                      </p>
                    )}
                  </div>
                )}
...
function VoucherRow({
  ...
  onSelect: (voucher: Voucher) => void | Promise<void>
}) {
      ...
      <div className="grid grid-cols-[64px_200px_minmax(0,1fr)] items-center gap-2 px-4 py-3 sm:grid-cols-[72px_210px_minmax(0,1fr)]">
        {type === 'purchases' ? (
          <label
            ...
            aria-label={`Select ${voucher.invoice_no}`}
          >
            <input
              type="checkbox"
              ...
              onChange={() =>
                setSelectedIds((ids) =>
                  ids.includes(voucher.id)
                    ? ids.filter((id) => id !== voucher.id)
                    : [...ids, voucher.id],
                )
              }
              className="h-5 w-5"
        ...
        <button
          type="button"
          onClick={() => void onSelect(voucher)}
          className="min-w-0 text-left"
          aria-label={`Open ${voucher.invoice_no}`}
          title=""
        >
          <span className="block whitespace-nowrap text-sm font-black text-black underline decoration-slate-400 underline-offset-2">
            {voucher.invoice_no}
          </span>
          <span className="mt-1 block whitespace-nowrap text-[11px] font-semibold text-slate-600">
            {voucherDate(voucher)}
          </span>
        </button>
        ...
        <button
          ...
          onClick={() => void onSelect(voucher)}
          className="min-w-0 text-left"
          aria-label={`Open ${voucher.invoice_no} for ${party}`}
          title=""
        >
          <span className="block break-words text-sm font-black leading-5 text-black">
            {party}
          </span>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <span className={strong ? 'font-black text-black' : 'font-bold text-slate-700'}>
        {label}
      </span>
      <span className={strong ? 'text-lg font-black text-black' : 'font-black text-black'}>
        {value}
      </span>
    </div>
  )
