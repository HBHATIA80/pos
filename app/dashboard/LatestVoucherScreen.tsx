      ...
      <div className="mx-auto flex h-full max-h-[92vh] w-full flex-col overflow-hidden rounded-none bg-white shadow-2xl sm:my-2 sm:rounded-3xl">
        <header className="shrink-0 border-b border-slate-200 bg-white px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => setOpen(false)} className="rounded-xl p-2 text-slate-700 hover:bg-slate-100" aria-label="Close"><X className="h-5 w-5" /></button>
          </div>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[350px_minmax(0,1fr)]">
          <section className="min-h-0 overflow-x-hidden overflow-y-auto border-b border-slate-200 lg:border-b-0 lg:border-r" aria-label="Latest vouchers">
            <div className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50 px-4 py-3 text-[11px] font-black uppercase tracking-wider text-black">Select <span className="mx-2">·</span> Voucher <span className="mx-2">·</span> Supplier</div>
            {loading ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-emerald-700" /></div> : latest.map(voucher => (
              <div key={voucher.id} className={`border-b border-slate-200 ${selected?.id === voucher.id ? 'bg-emerald-50' : 'bg-white'} hover:bg-emerald-50`}>
                <div className="grid grid-cols-[64px_200px_minmax(0,1fr)] items-center gap-2 px-4 py-3 sm:grid-cols-[72px_210px_minmax(0,1fr)]">
                  ...
                  <button type="button" onClick={() => void selectVoucher(voucher)} className="min-w-0 text-left" aria-label={`Open ${voucher.invoice_no} for ${partyName(voucher, type === 'sales' ? 'Walk-in Customer' : 'Walk-in / Other')}`}>
                    ...
                    <span className="mt-1 flex min-w-0 items-center justify-between gap-2"><span className="truncate text-xs font-black text-black">{money(voucher.grand_total)}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${voucher.status === 'draft' ? 'bg-amber-100 text-amber-900' : 'bg-emerald-100 text-emerald-900'}`}>{voucher.status}</span></span>
                  </button>
                </div>
              </div>
            ))}
          ...
          <section className="min-h-0 overflow-y-auto bg-slate-50/60 p-4 sm:p-6" aria-label="Voucher details">
            {!selected ? <div className="flex h-full items-center justify-center text-sm font-bold text-slate-500">Select a voucher to view all details.</div> : (
              <div className="mx-auto max-w-4xl space-y-4">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-slate-500">{type === 'sales' ? 'Sales Invoice' : 'Purchase Invoice'}</p><h2 className="mt-1 text-2xl font-black tracking-tight text-black">{selected.invoice_no}</h2></div><span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black uppercase ${selected.status === 'draft' ? 'bg-amber-100 text-amber-900' : 'bg-emerald-100 text-emerald-900'}`}><CheckCircle2 className="h-3.5 w-3.5" />{selected.status}</span></div>
                  <div className="mt-5 grid gap-3 sm:grid-cols-3"><Info icon={<CalendarDays />} label="Invoice date" value={voucherDate(selected)} /><Info icon={<UserRound />} label={type === 'sales' ? 'Customer' : 'Supplier'} value={partyName(selected, type === 'sales' ? 'Walk-in Customer' : 'Walk-in / Other')} /><Info icon={<Phone />} label="Mobile" value={partyPhone(selected) || 'Not provided'} /></div>
                </div>

                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="flex items-center gap-2 border-b border-slate-200 px-5 py-4"><Package className="h-4 w-4 text-emerald-700" /><h3 className="text-sm font-black text-black">Invoice Items</h3><span className="text-xs font-bold text-slate-500">{selected.items?.length || 0} items</span></div>
                  {selected.items?.length ? (
                    <div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-wider text-black"><tr><th className="px-5 py-3 text-left">Product</th><th className="px-3 py-3 text-left">SKU</th><th className="px-3 py-3 text-right">Qty</th><th className="px-3 py-3 text-right">Unit Price</th><th className="px-5 py-3 text-right">Line Total</th></tr></thead><tbody className="divide-y divide-slate-100">
                      {selected.items.map(item => {
                        const hasDiscount = Number(item.discount_amount || 0) > 0
                        return <tr key={item.id}><td className="px-5 py-3 font-black text-black"><span>{item.product_name || 'Unnamed product'}</span>{item.unit_name ? <span className="ml-1 text-xs font-semibold text-slate-500">({item.unit_name})</span>}{hasDiscount ? <div className="text-[10px] font-bold text-slate-500">Discount: {money(Number(item.discount_amount))}</div> : null}</td><td className="px-3 py-3 font-bold text-black">{item.sku || '—'}</td><td className="px-3 py-3 text-right font-black text-black">{item.quantity}</td><td className="px-3 py-3 text-right font-bold text-black">{money(item.unit_price)}</td><td className="px-5 py-3 text-right font-black text-black">{money(item.line_total)}</td></tr>
                      })}
                    </tbody></table></div>
