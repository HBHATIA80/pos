...
export default function OrdersPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-4 pb-3">
    <section className="overflow-hidden rounded-[28px] bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 p-5 text-white shadow-lg sm:p-7">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="max-w-2xl"><div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold ring-1 ring-white/20"><ShoppingCart className="h-3.5 w-3.5" /> Customer shopping</div><h1 className="mt-3 text-2xl font-black tracking-tight sm:text-4xl">Order what you need, in seconds.</h1><p className="mt-2 text-sm leading-6 text-indigo-100 sm:text-base">Search products by name, SKU, barcode, category, subcategory or brand.</p></div><div className="flex gap-2"><Link href="/dashboard/marketplace" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-white px-4 text-sm font-bold text-indigo-700 shadow-sm hover:bg-indigo-50"><Store className="h-4 w-4" /> Marketplace</Link><Link href="/dashboard/my-ledger" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-white/15 px-4 text-sm font-bold text-white ring-1 ring-white/20 hover:bg-white/20"><WalletCards className="h-4 w-4" /> My ledger</Link></div></div>
    </section>
    <section className="sticky top-16 z-20 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-sm backdrop-blur sm:p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input autoFocus value={search} onChange={event => setSearch(event.target.value)} placeholder="Search product name, SKU or barcode…" className="min-h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-12 pr-10 text-base outline-none focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-100" />{search && <button type="button" onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl p-2 text-slate-400" aria-label="Clear search"><X className="h-4 w-4" /></button>}</div><label className="relative flex min-w-0 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 md:w-64"><Store className="h-5 w-5 shrink-0 text-indigo-600" /><select value={shopId} onChange={event => setShopId(event.target.value)} className="min-h-10 min-w-0 flex-1 appearance-none bg-transparent pr-7 text-sm font-semibold outline-none"><option value="">Select shop</option>{shops.map(shop => <option key={shop.business_id} value={shop.business_id}>{shop.business?.name || 'Shop'}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 h-4 w-4 text-slate-400" /></label></div>
