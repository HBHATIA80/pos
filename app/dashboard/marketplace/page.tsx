...
import { ArrowLeft, ChevronDown, Loader2, PackageSearch, Search, Store, X } from 'lucide-react'
...
export default function MarketplacePage() {
  ...
  async function load(q = search, append = false) {
    ...
    try {
      ...
      if (!categories.length && body.products?.length) {
        const categoryMap = new Map<string, Facet>(); const subMap = new Map<string, Facet>(); const brandMap = new Map<string, Facet>()
        for (const item of body.products as MarketplaceProduct[]) {
          if (item.category_id && item.category_name) categoryMap.set(item.category_id, { id: item.category_id, name: item.category_name })
          if (item.subcategory_id && item.subcategory_name) subMap.set(item.subcategory_id, { id: item.subcategory_id, name: item.subcategory_name, category_id: item.category_id || undefined })
          if (item.brand_id && item.brand_name) brandMap.set(item.brand_id, { id: item.brand_id, name: item.brand_name })
        }
        setCategories([...categoryMap.values()].sort((a,b) => a.name.localeCompare(b.name)))
        setSubcategories([...subMap.values()].sort((a,b) => a.name.localeCompare(b.name)))
        setBrands([...brandMap.values()].sort((a,b) => a.name.localeCompare(b.name)))
      }
