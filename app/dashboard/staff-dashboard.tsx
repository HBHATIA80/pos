...
import { Banknote, BarChart3, CalendarDays, ChevronRight, CreditCard, RefreshCw, ShoppingCart, WalletCards, X } from 'lucide-react'
...
type DetailKey = 'today' | 'sales' | 'cash' | 'bank' | 'collections'
...
export default function StaffDashboard() {
  ...
  return <div className="min-w-0 space-y-5 pb-8">
    ...
    <section className="grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-5">
      <Metric icon={<CalendarDays />} label="Today" value={t.totalCollections} tone="yellow" subtext={todayLabel()} onClick={() => setDetailKey('today')} />
...
function DetailModal({ data, detailKey, onClose }: { data: Report; detailKey: DetailKey; onClose: () => void }) {
  const title = detailKey === 'today' ? 'Today&apos;s overview' : detailKey === 'sales' ? 'Today&apos;s sales' : detailKey === 'cash' ? 'Cash collections' : detailKey === 'bank' ? 'Bank collections' : 'Total collections'
  const value = detailKey === 'today' ? data.today.totalCollections : detailKey === 'sales' ? data.today.sales : detailKey === 'cash' ? data.today.cashCollections : detailKey === 'bank' ? data.today.bankCollections : data.today.totalCollections
  ...
  return <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
    <div className="flex min-h-0 w-full max-w-[1100px] max-h-[calc(100vh-32px)] flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-white p-5 sm:p-6">
        {detailKey === 'today' ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Summary label="Today&apos;s sales" value={money(data.today.sales)} tone="yellow" /><Summary label="Cash received" value={money(data.today.cashCollections)} tone="green" /><Summary label="Bank received" value={money(data.today.bankCollections)} tone="red" /><Summary label="Total collection" value={money(data.today.totalCollections)} tone="yellow" /></div> : <div className="grid gap-3 sm:grid-cols-3"><Summary label="Today" value={money(value)} /><Summary label="7-day sales" value={money(data.totals.sales)} /><Summary label="7-day collections" value={money(data.totals.collections)} /></div>}
...
function Summary({ label, value, tone = 'default' }: { label: string; value: string; tone?: 'default' | 'yellow' | 'green' | 'red' }) { const styles = tone === 'yellow' ? 'border-amber-200 bg-amber-50 text-amber-950' : tone === 'green' ? 'border-emerald-200 bg-emerald-50 text-emerald-950' : tone === 'red' ? 'border-rose-200 bg-rose-50 text-rose-950' : 'border-slate-200 bg-slate-50 text-slate-950'; return <div className={`rounded-2xl border p-4 ${styles}`}><p className="text-[10px] font-black uppercase tracking-[.12em] opacity-60">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div> }
function Metric({ icon, label, value, tone, subtext, onClick }: { icon: ReactNode; label: string; value: number; tone: 'green' | 'red' | 'yellow'; subtext?: string; onClick: () => void }) { const bg = tone === 'green' ? 'bg-emerald-50 text-emerald-700' : tone === 'red' ? 'bg-rose-50 text-rose-600' : 'bg-amber-100 text-amber-700'; const border = tone === 'yellow' ? 'hover:border-amber-300 focus:ring-amber-500/30' : 'hover:border-emerald-200 focus:ring-emerald-500/30'; return <button type="button" onClick={onClick} className={`group min-w-0 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 ${border}`}><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${bg}`}>{icon}</span><p className="mt-3 text-[11px] font-black uppercase tracking-[.12em] text-slate-400">{label}</p><p className="mt-1 text-2xl font-black tracking-tight text-slate-950">{money(value)}</p>{subtext ? <p className="mt-1 truncate text-[11px] font-semibold text-amber-700">{subtext}</p> : null}<span className="mt-2 block text-[11px] font-bold text-emerald-700 opacity-70 group-hover:opacity-100">View full details →</span></button> }
...
function DashboardSkeleton() { return <div className="space-y-5"><div className="h-48 animate-pulse rounded-[30px] bg-slate-100" /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{[1,2,3,4,5].map(i => <div key={i} className="h-32 animate-pulse rounded-2xl bg-slate-100" />)}</div><div className="h-80 animate-pulse rounded-[26px] bg-slate-100" /></div> }