...
import { Check, History, PackageSearch, Save, Search, TrendingUp } from 'lucide-react'
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-start gap-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-700"><TrendingUp className="h-6 w-6" /></span><div><p className="text-xs font-black uppercase tracking-[.15em] text-blue-700">Pricing desk</p><h1 className="mt-1 text-2xl font-extrabold text-slate-950">Item Pricing</h1><p className="mt-1 text-sm text-slate-600">Review current pricing, change the sale price and compare the last five completed purchase prices.</p></div></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(320px,420px)_1fr]">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-4"><div className="flex items-center gap-2"><PackageSearch className="h-5 w-5 text-blue-700" /><h2 className="text-lg font-extrabold text-slate-950">Products</h2><span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">{products.length}</span></div></div>
        <div className="max-h-[620px] overflow-y-auto">
          {loading ? <div className="p-8 text-center text-sm font-semibold text-slate-600">Loading products…</div> : products.length ? products.map(product => <button key={product.id} type="button" onClick={() => setSelected(product)} className={`w-full border-b border-slate-100 px-5 py-4 text-left transition hover:bg-blue-50 ${selected?.id === product.id ? 'bg-blue-50 ring-inset ring-2 ring-blue-200' : 'bg-white'}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-base font-extrabold text-slate-950">{product.name}</p><p className="mt-1 text-sm font-medium text-slate-600">{product.sku}{product.barcode ? ` · ${product.barcode}` : ''}</p></div><span className="shrink-0 text-sm font-extrabold text-blue-700">{money(product.sale_price)}</span></div></button>) : <div className="p-8 text-center text-sm font-semibold text-slate-600">No products found.</div>}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        {!selected || !detail ? <div className="flex min-h-[420px] items-center justify-center text-sm font-semibold text-slate-600">Select an item to see its pricing.</div> : <>
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black uppercase tracking-[.14em] text-blue-700">Selected item</p><h2 className="mt-1 text-2xl font-extrabold text-slate-950">{detail.product.name}</h2><p className="mt-1 text-sm font-semibold text-slate-600">SKU {detail.product.sku} · Stock {detail.product.current_stock}</p></div><div className="rounded-xl bg-blue-50 px-4 py-3 text-right"><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Current sale price</p><p className="mt-1 text-2xl font-black text-blue-700">{money(detail.product.sale_price)}</p></div></div>
          ...
          <div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50/60 p-5"><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div className="w-full md:max-w-sm"><label className="text-sm font-extrabold text-slate-950">Sale price</label><div className="mt-2 flex"><span className="flex h-12 items-center rounded-l-xl border border-r-0 border-slate-300 bg-white px-4 text-base font-bold text-slate-700">₹</span><input type="number" min="0" step="0.01" value={salePrice} onChange={e => setSalePrice(e.target.value)} className="h-12 w-full rounded-r-xl border border-slate-300 bg-white px-4 text-base font-extrabold text-slate-950" /></div></div><button type="button" disabled={saving} onClick={() => void saveSalePrice()} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 font-extrabold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60"><Save className="h-5 w-5" />{saving ? 'Saving…' : 'Save Sale Price'}</button></div><p className="mt-3 text-sm font-medium text-slate-600">This updates the product master sale price used by the POS when no customer-specific price list overrides it.</p></div>

          <div className="mt-7"><div className="flex items-center gap-2"><History className="h-5 w-5 text-blue-700" /><h3 className="text-lg font-extrabold text-slate-950">Last 5 purchase prices</h3></div><div className="mt-3 overflow-x-auto rounded-xl border border-slate-200"><table><thead><tr><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Purchase</th><th className="px-4 py-3 text-right">Qty</th><th className="px-4 py-3 text-right">Unit price</th></tr></thead><tbody>{detail.last_purchase_prices.length ? detail.last_purchase_prices.map(row => <tr key={row.id}><td className="px-4 py-3">{new Date(row.purchased_at).toLocaleDateString('en-IN')}</td><td className="px-4 py-3 font-bold text-slate-950">{row.invoice_no}</td><td className="px-4 py-3 text-right">{row.quantity}</td><td className="px-4 py-3 text-right font-extrabold text-slate-950">{money(row.unit_price)}</td></tr>) : <tr><td colSpan={4} className="px-4 py-8 text-center font-semibold text-slate-600">No completed purchase history found for this item.</td></tr>}</tbody></table></div></div>
        </>}
      </section>
    </div>
  </div>
