  ...
  >
    ...
    {invoice.physical_verified_at && (
      <p className="mt-3 border-t border-black/5 pt-3 text-xs font-medium text-slate-500">
        ...
        <div className="grid min-w-0 gap-3 rounded-2xl border border-green-100 bg-green-50/60 p-4 sm:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_minmax(0,0.8fr)]">
          <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{invoice.kind === 'purchase' ? 'Supplier' : 'Customer'}</p><p className="mt-1 break-words font-black text-slate-900">{invoice.party?.name || 'Walk-in / Other'}</p>{invoice.party?.phone && <p className="break-words text-xs font-medium text-slate-500">{invoice.party.phone}</p>}</div>
          <div className="min-w-0">{editing ? <><label className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Invoice Date & Time</label><input type="datetime-local" value={editDate} onChange={(e) => setEditDate(e.target.value)} className="mt-1 w-full min-w-0 rounded-lg border border-green-200 bg-white px-2 py-2 text-sm font-semibold outline-none focus:border-green-600" /></> : <><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Date</p><p className="mt-1 break-words font-semibold text-slate-900">{new Date(invoice.date || invoice.created_at).toLocaleString('en-IN')}</p></>}</div>
          <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Status</p><span className="mt-1 inline-flex max-w-full rounded-full bg-green-100 px-2.5 py-1 text-[10px] font-black uppercase text-green-800">{invoice.status}</span></div>
        </div>

        {editing && <div className="relative mt-4 min-w-0 rounded-2xl border border-slate-200 bg-white p-3"><label className="mb-2 block text-[10px] font-black uppercase tracking-wide text-slate-500">Add item</label><div className="flex min-w-0 items-center gap-2"><div className="relative min-w-0 flex-1"><input value={productSearch} onChange={(e) => void searchProducts(e.target.value)} placeholder="Search product by name or SKU..." className="h-10 w-full min-w-0 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-green-600" />{productSearch.length >= 2 && <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-56 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl">{productsLoading ? <div className="p-3 text-sm text-slate-500">Searching...</div> : products.length ? products.map((product) => <button key={product.id} type="button" onClick={() => addProduct(product)} className="flex w-full items-center justify-between gap-3 border-b border-slate-100 px-3 py-2.5 text-left hover:bg-green-50"><span className="min-w-0 break-words"><b>{product.name}</b><span className="ml-2 text-xs text-slate-500">{product.sku}</span></span><span className="shrink-0 text-xs font-bold text-slate-600">{money(invoice.kind === 'purchase' ? product.purchase_price : product.sale_price)}</span></button>) : <div className="p-3 text-sm text-slate-500">No products found</div>}</div>}</div><span className="inline-flex h-10 shrink-0 items-center gap-1 rounded-xl bg-green-50 px-3 text-xs font-bold text-green-800"><Plus className="h-4 w-4" /> Add</span></div></div>}

        <div className="mt-4 min-w-0 overflow-x-auto rounded-2xl border border-slate-200">
        ...
        {editing && <div className="mt-4 min-w-0"><label className="mb-2 block text-xs font-bold uppercase tracking-wide text-slate-500">Notes</label><textarea value={editNotes} onChange={(e) => setEditNotes(e.target.value)} rows={3} className="w-full min-w-0 rounded-2xl border border-slate-200 p-3 text-sm outline-none focus:border-green-600" placeholder="Invoice notes" /></div>}
        <div className="mt-5 ml-auto w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-sm"><div className="flex justify-between"><span className="text-slate-500">Subtotal</span><span className="font-semibold">{money(editing ? editSubtotal : invoice.subtotal)}</span></div><div className="mt-2 flex justify-between"><span className="text-slate-500">Discount</span><span className="font-semibold">{money(editing ? editDiscount : invoice.discount_amount)}</span></div><div className="mt-3 flex justify-between rounded-xl bg-amber-100 px-3 py-2.5 text-base"><span className="font-black">Grand Total</span><span className="text-xl font-black">{money(editing ? editTotal : invoice.grand_total)}</span></div></div>
        {invoice.notes && !editing && <div className="mt-4 min-w-0 break-words rounded-2xl bg-amber-50 p-4 text-sm text-amber-900"><b>Notes:</b> {invoice.notes}</div>}
