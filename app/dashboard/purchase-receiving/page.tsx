...
export default function PurchaseReceivingPage() {
  ...
  return <div className="mx-auto max-w-[1680px] space-y-4 pb-24">
    ...
    <SplitPane className={`${styles.receivingLayout} grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_560px]`}>
      <section className={`${styles.receivingList} overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm`}>
        <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-black text-slate-950">Purchase Vouchers</h2><p className="text-xs font-semibold text-slate-700">Verify quantity received before closing the physical check.</p></div><div className="flex h-10 items-center gap-2 rounded-xl border border-slate-300 bg-slate-50 px-3 sm:w-72"><Search className="h-4 w-4 text-slate-600" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search invoice or supplier" className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-slate-950 outline-none placeholder:text-slate-500" /></div></div>
        <ScrollPanel className={`${styles.receivingListBody} max-h-[calc(100vh-300px)]`}>
          {loading ? <div className="p-10 text-center text-sm font-bold text-slate-700">Loading purchase vouchers…</div> : !filtered.length ? <div className="p-10 text-center text-sm font-bold text-slate-700">No purchase vouchers found.</div> : <div className="divide-y divide-slate-100">{filtered.map(purchase => { const receipt = receiptOf(purchase); const verified = receipt?.status === 'verified'; return <button key={purchase.id} onClick={() => openPurchase(purchase)} className={`${styles.receivingRow} flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-emerald-50 ${selected?.id === purchase.id ? 'bg-emerald-50 ring-inset ring-2 ring-emerald-200' : ''}`}><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${verified ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{verified ? <CheckCircle2 className="h-5 w-5" /> : <ShieldAlert className="h-5 w-5" />}</span><span className="min-w-0 flex-1"><span className="flex min-w-0 flex-wrap items-center gap-2"><span className="shrink-0 font-black text-slate-950">{purchase.invoice_no}</span>{!verified && <span className="rounded-md bg-amber-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-amber-800">Physical Product not checked with Invoice Still</span>}</span><span className={`${styles.receivingRowMeta} mt-0.5 block text-xs font-semibold text-slate-700`}>{partyName(purchase)} · {new Date(purchase.purchased_at || purchase.created_at).toLocaleDateString('en-IN')}</span></span><span className="shrink-0 text-right"><span className="block text-sm font-black text-slate-950">{money(purchase.grand_total)}</span><span className={`text-[10px] font-black uppercase ${verified ? 'text-emerald-700' : 'text-amber-700'}`}>{verified ? 'Verified' : receipt?.status === 'partial' ? 'Partial' : 'Not checked'}</span></span></button> })}</div>}
        </ScrollPanel>
      </section>
