...
export default function CustomerCatalogGuard({ role }: { role: string }) {
  ...
  useEffect(() => {
    // Customer-catalog routing is required only by the customer Shop & Orders
    // portal. Do not install this fetch override across the business workspace.
    if (role !== 'user' || !pathname.startsWith('/dashboard/orders')) return
