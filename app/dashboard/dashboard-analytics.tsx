...
export default function DashboardAnalytics() {
  ...
  return <div className="space-y-5 pb-8">
    <section className="relative overflow-hidden rounded-[30px] border border-emerald-200 bg-gradient-to-br from-[#f0fbf4] via-[#e2f5e8] to-[#ccebd8] p-5 shadow-[0_14px_40px_rgba(22,101,52,.08)] sm:p-7">
      ...
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div className="max-w-3xl"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.16em] text-emerald-800"><Sparkles className="h-3.5 w-3.5" /> Live business pulse</span><h1 className="mt-4 text-2xl font-black tracking-[-.04em] text-slate-950 sm:text-[32px]"><Greeting /> — here&apos;s your business at a glance.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Sales, inventory purchases, operating expenses and profit are separated correctly. Profit uses moving weighted-average purchase cost from completed purchases.</p><div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-600"><span className="rounded-full bg-white/85 px-3 py-1.5 ring-1 ring-emerald-100">{fullDate(data.period.start)} – {fullDate(data.period.end)}</span><span className="rounded-full bg-white/85 px-3 py-1.5 ring-1 ring-emerald-100">Live data</span></div></div><div className="flex shrink-0 flex-wrap gap-2">{[7,14,30].map(value => <button key={value} onClick={() => changePeriod(value as 7 | 14 | 30)} className={`rounded-xl px-4 py-2.5 text-xs font-black transition ${days === value ? 'bg-slate-950 text-white shadow-md' : 'bg-white/90 text-slate-800 ring-1 ring-emerald-100 hover:bg-emerald-50 hover:text-slate-950'}`}>{value} days</button>)}<button onClick={() => void load(days, true)} disabled={refreshing} className="rounded-xl bg-white/90 p-2.5 text-slate-800 ring-1 ring-emerald-100 transition hover:bg-emerald-50 hover:text-slate-950 disabled:opacity-60" title="Refresh dashboard"><RefreshCw className={refreshing ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} /></button></div></div>
...
function HeroMetric({icon,label,value,tone='default',onClick}:{icon:ReactNode;label:string;value:number;tone?:'default'|'positive'|'negative';onClick:()=>void}){return <button onClick={onClick} className="group min-h-[112px] rounded-2xl border border-white/80 bg-white/70 p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50/90 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-200"><div className="flex items-start justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-50 text-emerald-700 transition-colors group-hover:bg-white group-hover:text-emerald-800">{icon}</span><ChevronRight className="h-4 w-4 text-slate-400 transition-colors group-hover:text-emerald-700"/></div><p className="mt-3 text-[10px] font-black uppercase tracking-[.14em] text-slate-500 transition-colors group-hover:text-emerald-800">{label}</p><p className={`mt-1 text-xl font-black ${tone==='negative'?'text-red-600':tone==='positive'?'text-emerald-700':'text-slate-950'}`}>{money(value)}</p></button>}
function Kpi({icon,label,value,hint,tone,onClick,featured=false}:{icon:ReactNode;label:string;value:number;hint:string;tone:'green'|'red';onClick:()=>void;featured?:boolean}){return <button onClick={onClick} className={`group rounded-[22px] border p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50/80 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-200 ${featured?'border-emerald-200 bg-emerald-50/70':'border-slate-200 bg-white'}`}><div className="flex items-start justify-between"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tone==='red'?'bg-red-50 text-red-700':'bg-emerald-50 text-emerald-700'}`}>{icon}</span><ChevronRight className="h-4 w-4 text-slate-400 transition-colors group-hover:text-emerald-700"/></div><p className="mt-3 text-[10px] font-black uppercase tracking-[.14em] text-slate-500 group-hover:text-emerald-800">{label}</p><p className={`mt-1 text-xl font-black ${tone==='red'&&value>0?'text-red-600':'text-slate-950'}`}>{money(value)}</p><p className="mt-1 text-[11px] text-slate-500 group-hover:text-slate-600">{hint}</p></button>}
...
function DetailModal({type,detail,loading,selectedInvoice,setSelectedInvoice,partyName,onClose}:{type:string;detail:Detail|null;loading:boolean;selectedInvoice:Invoice|null;setSelectedInvoice:(invoice:Invoice|null)=>void;partyName:(invoice:Invoice)=>string;onClose:()=>void}){const title=type==='grossprofit'?'Gross profit':type==='netprofit'?'Net profit / loss':type==='sales'?'Sales invoices':type==='purchases'?'Purchase invoices':type==='expenses'?'Operating expenses':type==='stock'?'Stock at cost':type==='cashbank'?'Cash & bank activity':type==='receivables'?'Receivables':'Payables';return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 p-2 backdrop-blur-sm sm:p-4" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="flex max-h-[82vh] w-full max-w-3xl min-w-0 flex-col overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-2xl"><div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-2.5 sm:px-5"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.18em] text-emerald-700">Dashboard details</p><h2 className="mt-0.5 truncate text-base font-black text-slate-950">{selectedInvoice?selectedInvoice.invoice_no:title}</h2>{selectedInvoice&&<p className="truncate text-[10px] text-slate-500">{partyName(selectedInvoice)} · {formatDateTime(selectedInvoice.sold_at||selectedInvoice.created_at)}</p>}</div><button onClick={onClose} className="ml-3 rounded-lg border border-transparent p-1.5 text-slate-500 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800" aria-label="Close"><X className="h-4 w-4"/></button></div>{selectedInvoice?<InvoiceProfit invoice={selectedInvoice} onBack={()=>setSelectedInvoice(null)}/>:<div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">{loading?<div className="grid place-items-center py-20 text-sm font-semibold text-slate-500"><RefreshCw className="mb-2 h-5 w-5 animate-spin text-emerald-600"/>Loading details…</div>:detail?.rows?.[0]?.error?<div className="rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{detail.rows[0].error}</div>:<DetailRows type={type} rows={detail?.rows||[]} setSelectedInvoice={setSelectedInvoice}/>}</div>}</div></div>}

function DetailRows({type,rows,setSelectedInvoice}:{type:string;rows:any[];setSelectedInvoice:(invoice:Invoice|null)=>void}){if(!rows.length)return <div className="grid place-items-center py-16 text-center"><p className="font-black text-slate-800">No records found</p><p className="mt-1 text-sm text-slate-500">There is no data for this period.</p></div>;if(type==='grossprofit'||type==='netprofit')return <div className="overflow-hidden rounded-xl border border-slate-200"><div className="hidden grid-cols-[1.2fr_1fr_.7fr_.7fr] gap-3 bg-slate-50 px-3 py-2.5 text-[9px] font-black uppercase tracking-wider text-slate-500 sm:grid"><span>Invoice</span><span>Party / date</span><span className="text-right">Sales</span><span className="text-right">Profit / loss</span></div>{rows.map((row,index)=>row.kind==='sale'?<button key={`${row.id}-${index}`} onClick={()=>setSelectedInvoice(row.invoice)} className="group grid w-full gap-1.5 border-b border-slate-100 px-3 py-3 text-left transition last:border-0 hover:bg-emerald-50/80 hover:text-slate-950 sm:grid-cols-[1.2fr_1fr_.7fr_.7fr] sm:items-center sm:gap-3"><div><p className="text-sm font-black text-emerald-800 group-hover:text-emerald-900 underline decoration-emerald-300 underline-offset-2">{row.invoice_no}</p><p className="mt-0.5 text-[10px] text-slate-500">Click for item-wise weighted cost</p></div><div className="text-[11px]"><p className="font-semibold">{Array.isArray(row.party)?row.party[0]?.name:row.party?.name||'Walk-in / Other'}</p><p className="mt-0.5 text-slate-500">{formatDateTime(row.date)}</p></div><div className="text-right text-sm font-bold">{money(row.sales)}</div><div className={`text-right text-sm font-black ${row.gross_profit>=0?'text-emerald-600':'text-red-600'}`}>{money(row.gross_profit)}</div></button>:<div key={`${row.id}-${index}`} className="grid gap-1.5 border-b border-slate-100 px-3 py-3 sm:grid-cols-[1.2fr_1fr_.7fr_.7fr] sm:items-center sm:gap-3"><div className="text-sm font-black">{row.reference_id||'Expense'}</div><div className="text-[11px] text-slate-500">{row.account||row.description}</div><div/><div className="text-right text-sm font-black text-red-600">{money(-Math.abs(row.amount||0))}</div></div>)}</div>;
if(type==='sales'||type==='purchases')return <div className="space-y-1.5">{rows.map(row=><button key={row.id} type="button" onClick={()=>setSelectedInvoice(row as Invoice)} className="group flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-left transition hover:border-emerald-200 hover:bg-emerald-50/80 hover:shadow-sm"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900 group-hover:text-emerald-900">{row.invoice_no}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">{row.party?.name||(Array.isArray(row.parties)?row.parties[0]?.name:row.parties?.name)||'Walk-in / Other'} · {formatDateTime(row.sold_at||row.purchased_at||row.created_at)}</p></div><div className="ml-3 shrink-0 text-right"><p className="text-sm font-black text-slate-900">{money(row.grand_total)}</p><p className="text-[9px] font-bold uppercase text-slate-400 group-hover:text-emerald-700">Open invoice</p></div></button>)}</div>;
if(type==='expenses')return <div className="space-y-1.5">{rows.map(row=><div key={row.id} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 transition hover:border-emerald-200 hover:bg-emerald-50/80"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-900">{row.expense_no||row.category}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">{row.description||'Operating expense'} · {formatDateTime(row.expense_date||row.created_at)}</p></div><p className="ml-3 shrink-0 text-sm font-black text-red-600">{money(row.amount)}</p></div>)}</div>;
if(type==='stock')return <div className="overflow-hidden rounded-xl border border-slate-200"><table className="w-full table-fixed text-xs"><colgroup><col className="w-[45%]"/><col className="w-[15%]"/><col className="w-[20%]"/><col className="w-[20%]"/></colgroup><thead className="bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500"><tr><th className="px-3 py-2.5 text-left">Product</th><th className="px-3 py-2.5 text-right">Qty</th><th className="px-3 py-2.5 text-right">Purchase / pc</th><th className="px-3 py-2.5 text-right">Stock cost</th></tr></thead><tbody>{rows.map(row=><tr key={row.product_id} className="border-t border-slate-100 transition hover:bg-emerald-50/70"><td className="break-words px-3 py-2.5 font-bold text-slate-800">{row.name}<div className="text-[10px] font-normal text-slate-500">{row.sku}</div></td><td className="px-3 py-2.5 text-right font-bold">{quantity(row.current_stock)}</td><td className="px-3 py-2.5 text-right">{money(row.purchase_price,2)}</td><td className="px-3 py-2.5 text-right font-black">{money(row.stock_cost_value)}</td></tr>)}</tbody></table></div>;
if(type==='receivables'||type==='payables')return <div className="space-y-1.5">{rows.map(row=><div key={row.party_id} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 transition hover:border-emerald-200 hover:bg-emerald-50/70"><div className="min-w-0"><p className="truncate text-sm font-black">{row.name}</p><p className="mt-0.5 text-[11px] text-slate-500">{type==='receivables'?'Customer money due':'Supplier money due'}</p></div><p className="ml-3 shrink-0 text-sm font-black">{money(row.amount)}</p></div>)}</div>;
return <div className="space-y-1.5">{rows.map(row=><div key={row.id} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 transition hover:border-emerald-200 hover:bg-emerald-50/70"><div className="min-w-0"><p className="truncate text-sm font-black">{row.account||row.description||'Cash / Bank'}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">{row.reference_id||row.voucher_no||formatDateTime(row.entry_date)}</p></div><p className="ml-3 shrink-0 text-sm font-black">{money((row.debit||0)-(row.credit||0))}</p></div>)}</div>}
...
function InvoiceProfit({invoice,onBack}:{invoice:Invoice;onBack:()=>void}){
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
    <div className="mb-3 flex items-center justify-between gap-3">
      <button onClick={onBack} className="rounded-lg px-2.5 py-1.5 text-[11px] font-black text-slate-600 transition hover:bg-emerald-50 hover:text-emerald-800">← Back to invoices</button>
      <span className={`rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-wide ${invoice.status==='draft'?'bg-amber-50 text-amber-700':'bg-emerald-50 text-emerald-700'}`}>{invoice.status || 'posted'}</span>
    </div>
      ...
      <table className="w-full min-w-0 table-fixed border-collapse text-[11px] leading-4 sm:text-[12px]">
        ...
        <tbody>
          {items.map(item=>{
            ...
            return <tr key={item.id} className="border-t border-slate-100 align-middle transition hover:bg-emerald-50/80">
              <td className="min-w-0 px-2 py-2 sm:px-2.5 sm:py-2.5">
                <p className="break-words [overflow-wrap:anywhere] text-[11px] font-black leading-[1.25] text-slate-800 sm:text-[12px]">{item.product_name}</p>
              ...
              <td className="min-w-0 whitespace-nowrap px-0.5 py-2 text-right text-[10px] font-semibold text-slate-700 sm:px-1.5 sm:text-[11px]">{money(costPc,2)}</td>
              <td className="min-w-0 whitespace-nowrap px-0.5 py-2 text-right text-[10px] font-semibold text-slate-700 sm:px-1.5 sm:text-[11px]">{money(sellPc,2)}</td>
              <td className={`min-w-0 whitespace-nowrap px-0.5 py-2 text-right text-[10px] font-black sm:px-1.5 sm:text-[11px] ${perPc>=0?'text-emerald-600':'text-red-600'}`}>{money(perPc,2)}</td>
              <td className={`min-w-0 whitespace-nowrap px-1 py-2 text-right text-[10px] font-black sm:px-1.5 sm:text-[11px] ${total>=0?'text-emerald-600':'text-red-600'}`}>{money(total,2)}</td>
            </tr>
          })}
        </tbody>
      </table>
