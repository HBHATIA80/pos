  ...
  return <div className="mx-auto flex min-h-[calc(100vh-88px)] max-w-[1680px] flex-col gap-3 pb-28 lg:pb-3">
    <section className="shrink-0 rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex min-h-12 flex-wrap items-center gap-2 px-3 py-2">
        <div className="flex items-center gap-2 pr-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-600 text-white"><ShoppingCart className="h-4 w-4" /></span><div className="leading-tight"><div className="text-sm font-black text-slate-900">Sales Invoice</div><div className="text-[10px] text-slate-500">{businessName}</div></div></div>
        <div className="hidden h-7 w-px bg-slate-200 sm:block" />
        <span className="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-wide text-emerald-700">Live POS</span><span className="hidden text-xs text-slate-400 lg:inline">•</span><span className="hidden text-xs text-slate-500 lg:inline">New Invoice</span>
        <div className="ml-auto flex items-center gap-2"><span className="hidden rounded-lg bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-500 md:inline">Press <b>/</b> for products</span><button type="button" onClick={() => void boot()} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 hover:bg-slate-50"><RefreshCw className="h-3.5 w-3.5" />Refresh</button></div>
      </div>
    </section>
    ...
    <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_250px]">
      <main className="flex min-h-0 min-w-0 flex-col gap-3">
        ...
        <section className="flex min-h-[600px] flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-slate-50/80 px-3 py-2.5"><div className="flex items-center gap-2"><h2 className="text-sm font-black text-slate-900">Invoice Items</h2><span className="rounded-md bg-violet-100 px-2 py-0.5 text-[10px] font-black text-violet-700">{itemCount} Qty</span>{selectedCustomer && <span className="hidden text-[11px] text-slate-500 sm:inline">• {selectedCustomer.name}</span>}</div><div className="flex items-center gap-1.5"><button type="button" onClick={() => setCart([])} disabled={!cart.length || saving} className="inline-flex h-8 items-center gap-1 rounded-lg border border-rose-200 bg-white px-2.5 text-[11px] font-bold text-rose-600 disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" />Clear</button><button type="button" onClick={() => void save('draft')} disabled={!cart.length || saving} className="hidden h-8 items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 text-[11px] font-bold text-slate-700 disabled:opacity-40 sm:inline-flex"><Pause className="h-3.5 w-3.5" />Hold</button></div></div>
      ...
      <aside className="max-xl:fixed max-xl:bottom-16 max-xl:left-2 max-xl:right-2 max-xl:z-40 xl:sticky xl:top-3 xl:self-start">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg max-xl:shadow-2xl">
          <div className="flex items-center justify-between bg-violet-700 px-3 py-2.5 text-white"><div><div className="text-[9px] font-black uppercase tracking-[.18em] text-violet-200">Bill Summary</div><div className="mt-0.5 text-xl font-black tracking-tight">{money(subtotal)}</div></div><div className="text-right text-[9px] text-violet-100">{itemCount} items</div></div>
          <div className="hidden space-y-2 px-3 py-3 text-xs xl:block"><div className="flex justify-between"><span className="text-slate-500">Subtotal</span><b>{money(subtotal)}</b></div><div className="flex justify-between"><span className="text-slate-500">Discount</span><b>₹0.00</b></div><div className="border-t border-dashed border-slate-200 pt-2"><div className="flex items-end justify-between"><span className="font-bold text-slate-700">Grand Total</span><span className="text-xl font-black text-violet-700">{money(subtotal)}</span></div></div></div>
