...
type CartItem = Product & { line_id: string; quantity: number | ''; unit_price: number; pricing_source: 'base' | 'price_list'; price_overridden: boolean }
...
export default function SalesPage() {
  ...
  function setQuantityDraft(lineId: string, value: string) {
  ...
  async function save(status: 'draft' | 'completed') {
    ...
    try {
      ...
      const response = await fetch('/api/sales', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: { party_id: customer || null, status, items, invoice_date: invoiceDate } }) })
    ...
    <section className="shrink-0 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50 shadow-sm">
      <div className="flex min-h-[82px] flex-wrap items-center gap-4 px-4 py-3 md:px-5">
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto">
          <label className="relative flex h-10 cursor-pointer items-center gap-2 rounded-xl border border-emerald-200 bg-white px-3 shadow-sm"><CalendarDays className="h-4 w-4 text-emerald-800" /><span className="text-xs font-black text-black">{displayDate}</span><input type="date" value={invoiceDate} onChange={event => setInvoiceDate(event.target.value)} aria-label="Invoice date" className="absolute inset-0 cursor-pointer opacity-0" /></label>
      ...
      <main className="flex min-h-0 min-w-0 flex-col gap-3">
        ...
        <section className="relative z-0 flex min-h-[520px] flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="min-h-0 flex-1 overflow-auto">
            {cart.length ? <table className="w-full min-w-[860px] text-sm"><thead className="sticky top-0 z-10 bg-white text-[11px] font-black uppercase tracking-wide text-slate-700"><tr><th className="w-[42%] px-4 py-3 text-left">Product</th><th className="px-2 py-3 text-center">Stock</th><th className="px-2 py-3 text-center">Qty</th><th className="px-2 py-3 text-right">Rate</th><th className="px-4 py-3 text-right">Amount</th><th className="w-10" /></tr></thead><tbody className="divide-y divide-slate-200">{cart.map(item => <tr key={item.line_id} className="hover:bg-emerald-50/40"><td className="px-4 py-3"><div className="flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100">{item.image_url ? <img src={item.image_url} alt="" className="h-full w-full object-cover" /> : <ImageIcon className="h-4 w-4 text-slate-500" />}</div><div className="min-w-0"><div className="truncate text-sm font-black text-black">{item.name}</div><div className="text-xs font-bold text-slate-600">{item.sku}{item.pricing_source === 'price_list' ? ' · Customer Price' : ''}</div></div></div></td><td className="px-2 py-3 text-center text-xs font-black text-slate-700">{item.current_stock}</td><td className="px-2 py-3"><div className="mx-auto flex h-12 w-[148px] items-center justify-between rounded-xl border border-slate-300 bg-white shadow-sm"><button type="button" onClick={() => setQuantityDraft(item.line_id, String(Math.max(1, Number(item.quantity || 1) - 1)))} className="flex h-full w-10 items-center justify-center rounded-l-xl hover:bg-slate-100"><Minus className="h-4 w-4" /></button><input type="text" inputMode="numeric" value={item.quantity} onChange={event => setQuantityDraft(item.line_id, event.target.value.replace(/\D/g, ''))} onBlur={() => void commitQuantity(item.line_id)} onKeyDown={event => { if (event.key === 'Enter') { event.currentTarget.blur() } }} className="h-full min-w-0 w-[76px] bg-transparent text-center text-lg font-black text-black outline-none" aria-label={`Quantity for ${item.name}`} autoComplete="off" /><button type="button" onClick={() => setQuantityDraft(item.line_id, String(Math.min(Number(item.current_stock), Number(item.quantity || 0) + 1)))} className="flex h-full w-10 items-center justify-center rounded-r-xl hover:bg-slate-100"><Plus className="h-4 w-4" /></button></div></td><td className="px-2 py-3 text-right"><input aria-label={`Rate for ${item.name}`} type="text" inputMode="decimal" value={item.unit_price} onChange={event => setPrice(item.line_id, event.target.value.replace(/[^0-9.]/g, ''))} className="h-11 w-36 rounded-xl border border-slate-300 bg-white px-3 text-right text-base font-black text-black outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 tabular-nums" /></td><td className="px-4 py-3 text-right text-sm font-black text-black">{money(Number(item.quantity || 0) * Number(item.unit_price))}</td><td className="px-2"><button type="button" onClick={() => setCart(current => current.filter(value => value.line_id !== item.line_id))} className="rounded-lg p-2 text-slate-500 hover:bg-rose-50 hover:text-rose-700"><X className="h-4 w-4" /></button></td></tr>)}</tbody></table> : <div className="flex h-full min-h-[500px] flex-col items-center justify-center px-6 text-center"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-800"><ShoppingCart className="h-8 w-8" /></div><h3 className="mt-4 text-lg font-black text-black">Start your sale</h3><p className="mt-1 max-w-sm text-sm font-medium text-slate-600">Choose a customer, then search or scan a product. Quantity and rate can be edited directly in the invoice.</p></div>}
          </div>
