...
export default function ProductImportPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    <div className="flex items-center gap-3">
      <a href="/dashboard/products" className="rounded-xl border p-2 hover:bg-slate-50"><ArrowLeft className="h-5 w-5" /></a>
      <div><p className="text-xs font-black uppercase tracking-[.18em] text-indigo-600">BIZBook Catalog</p><h1 className="text-3xl font-black">Import & Update Products</h1><p className="text-sm text-slate-500">Bulk update your shop catalog without manually re-entering existing products.</p></div>
    </div>

    <section className="rounded-3xl bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-4"><div className="rounded-2xl bg-white/15 p-3"><FileSpreadsheet className="h-7 w-7" /></div><div><h2 className="text-xl font-black">Bulk add products in an Excel-style window</h2><p className="mt-1 max-w-2xl text-sm text-indigo-100">Enter many products in one grid, paste rows directly from Excel, then validate and add them together.</p></div></div>
        <button type="button" onClick={() => setBulkOpen(true)} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-black text-indigo-700 shadow-sm"><Plus className="h-4 w-4" /> Open Bulk Product Entry</button>
      </div>
      <a href="/api/catalog/import/template" className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-xl bg-white/15 px-4 text-sm font-bold text-white hover:bg-white/20"><Download className="h-4 w-4" /> Download Shop Template</a>
    </section>
