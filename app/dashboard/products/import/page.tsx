...
export default function ProductImportPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-3xl bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 p-6 text-white shadow-xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-4"><div className="rounded-2xl bg-white/15 p-3"><FileSpreadsheet className="h-7 w-7" /></div><div><h2 className="text-xl font-black">Bulk add products in an Excel-style window</h2><p className="mt-1 max-w-2xl text-sm text-indigo-100">Enter many products in one grid, paste rows directly from Excel, then validate and add them together.</p></div></div><button type="button" onClick={() => setBulkOpen(true)} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-black text-indigo-700 shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-md"><Plus className="h-4 w-4" /> Open Bulk Product Entry</button></div><a href="/api/catalog/import/template" className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-xl bg-white/15 px-4 text-sm font-bold text-white hover:bg-white/20"><Download className="h-4 w-4" /> Download Shop Template</a></section>
    ...
    {bulkOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-3 sm:p-6" role="dialog" aria-modal="true" onMouseDown={event => { if (event.target === event.currentTarget && !newMaster) setBulkOpen(false) }}><div className="flex h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><div className="flex items-center gap-2"><FileSpreadsheet className="h-5 w-5 text-emerald-700" /><h2 className="text-xl font-black text-slate-950">Bulk Product Entry</h2></div><p className="mt-1 text-xs font-medium text-slate-600">Excel-style grid · paste from Excel · fields marked * are required · up to 5,000 products</p></div><button type="button" onClick={() => setBulkOpen(false)} className="rounded-xl border border-slate-300 bg-white p-2 text-slate-700 transition hover:bg-slate-100 hover:text-slate-950"><X className="h-5 w-5" /></button></div>
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-slate-50 px-5 py-3"><button type="button" onClick={() => addRows()} className="inline-flex min-h-10 items-center gap-2 rounded-xl border-2 border-emerald-800 bg-emerald-800 px-4 text-sm font-black text-white shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:border-emerald-800 hover:bg-white hover:text-emerald-900 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-300"><Plus className="h-4 w-4" /> Add 5 Rows</button><button type="button" onClick={downloadGrid} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 transition-all duration-150 hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-900 hover:shadow-sm"><Download className="h-4 w-4" /> Download Filled Excel</button><button type="button" onClick={clearSheet} className="min-h-10 rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 transition-all duration-150 hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-900 hover:shadow-sm">Clear All</button><span className="ml-auto text-sm font-bold text-slate-600">{filledRows.length} product{filledRows.length === 1 ? '' : 's'} ready</span></div>
      <div className="min-h-0 flex-1 overflow-auto" onPaste={handlePaste}><table className="min-w-[2100px] border-collapse text-sm"><thead className="sticky top-0 z-10 bg-slate-100"><tr><th className="sticky left-0 z-20 w-12 border border-slate-300 bg-slate-100 px-2 py-3 text-center font-black text-slate-800">#</th>{columns.map(column => <th key={column.key} className={`${column.width} border border-slate-300 bg-slate-100 px-3 py-3 text-left text-[12px] font-black uppercase tracking-wide text-slate-800`}>{column.label}</th>)}<th className="sticky right-0 z-40 w-16 border border-slate-300 bg-slate-100 px-1 text-center text-[11px] font-black uppercase tracking-wide text-slate-800">Action</th></tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="hover:bg-emerald-50/30"><td className="sticky left-0 z-10 border border-slate-300 bg-white px-2 py-1 text-center font-bold text-slate-600">{rowIndex + 1}</td>{columns.map((column, colIndex) => { const required = ['SKU', 'Product Name', 'Category', 'Brand', 'Unit'].includes(column.key); const invalid = required && filledRows.includes(row) && !row[column.key]?.trim(); const masterConfig = column.key === 'Category' ? ['Category', masters.categories, 'Select category'] as const : column.key === 'Subcategory' ? ['Subcategory', masters.subcategories, 'Select subcategory'] as const : column.key === 'Brand' ? ['Brand', masters.brands, 'Select brand'] as const : column.key === 'Unit' ? ['Unit', masters.units, 'Select unit'] as const : null; return <td key={column.key} className={`border border-slate-300 p-0 ${invalid ? 'bg-rose-50' : 'bg-white'}`}>{masterConfig ? renderMasterSelect(row, rowIndex, masterConfig[0], masterConfig[1], masterConfig[2]) : <input value={row[column.key] ?? ''} onFocus={() => setActiveCell({ row: rowIndex, col: colIndex })} onChange={event => updateCell(rowIndex, column.key, event.target.value)} className={`h-11 w-full border-0 bg-transparent px-3 text-sm font-medium text-slate-800 outline-none focus:bg-indigo-50 ${invalid ? 'ring-1 ring-inset ring-rose-400' : ''}`} placeholder={column.key === 'Active' ? 'Yes' : ''} />}</td> })}<td className="sticky right-0 z-40 border border-slate-300 bg-white p-1"><button type="button" onClick={() => deleteRow(rowIndex)} className="group flex h-10 w-10 items-center justify-center rounded-xl border-2 border-rose-600 bg-rose-50 text-rose-700 shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:border-rose-700 hover:bg-rose-100 hover:text-rose-800 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-rose-300" aria-label={`Delete row ${rowIndex + 1}`} title={`Delete row ${rowIndex + 1}`}><Trash2 className="h-5 w-5 !text-rose-700 !stroke-rose-700 group-hover:!text-rose-800 group-hover:!stroke-rose-800" strokeWidth={2.7} color="#b91c1c" /></button></td></tr>)}</tbody></table></div>
      <div className="flex flex-col gap-3 border-t border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs font-medium leading-5 text-slate-600">Tip: use the dropdowns for existing master data. Each dropdown also has <b className="text-slate-900">＋ Add new…</b> so missing category, subcategory, brand or unit can be created without leaving this window.</p><div className="flex items-center gap-2 sm:shrink-0"><button type="button" onClick={() => setBulkOpen(false)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-5 text-sm font-bold text-slate-700 transition hover:bg-slate-50">Cancel</button><button type="button" disabled={busy} onClick={() => void importGrid()} className="group min-h-11 rounded-xl border-2 border-emerald-800 bg-emerald-800 px-5 text-sm font-black text-white shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:border-emerald-800 hover:bg-white hover:text-emerald-900 hover:shadow-md disabled:cursor-not-allowed disabled:border-slate-300 disabled:bg-slate-100 disabled:text-slate-500 disabled:shadow-none">{busy ? 'Validating & adding…' : `Add ${filledRows.length} Product${filledRows.length === 1 ? '' : 's'}`}</button></div></div>
    </div></div>}
