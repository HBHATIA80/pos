...
type Result = { imported?: number; validRows?: number; error?: string; errors?: { row: number; message: string }[] }
...
const columns: Column[] = [
  ...
  { key: 'Barcode', label: 'Barcode', width: 'w-44' },
  ...
  { key: 'Subcategory', label: 'Subcategory', width: 'w-48' },
  ...
  { key: 'Sale Price', label: 'Sale price', width: 'w-40' },
  { key: 'Opening Stock', label: 'Opening stock', width: 'w-40' },
  { key: 'Current Stock', label: 'Current stock', width: 'w-40' },
  { key: 'Reorder Level', label: 'Reorder level', width: 'w-40' },
  { key: 'Active', label: 'Status', width: 'w-32' },
]
...
const draftKey = 'bizybuk.bulk-product-entry.v2'
...
export default function ProductImportPage() {
  ...
  async function postWorkbook(workbook: XLSX.WorkBook) {
    ...
    try {
      ...
      setResult(body)
      if (!response.ok) {
        toast.error(body.error || 'Import failed. Please review the errors below.')
        return
      }
      try { localStorage.removeItem(draftKey) } catch { /* ignore */ }
  ...
  async function importGrid() {
    ...
    if (validationErrors.length) {
      ...
      toast.error(`Please fix ${validationErrors.length} validation issue${validationErrors.length === 1 ? '' : 's'} before saving.`)
      return
    }
  ...
  async function upload() {
    ...
    try {
      ...
      else toast.success(body.message || 'Products imported successfully')
    } catch { toast.error('Unable to upload the Excel file.') }
    finally { setBusy(false) }
  }
  ...
  return <div className="mx-auto max-w-6xl space-y-6 pb-10">
    ...
    {bulkOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-sm sm:p-6" onMouseDown={event => { if (event.target === event.currentTarget && !busy) setBulkOpen(false) }}><div className="flex max-h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Bulk product entry"><div className="border-b border-slate-200 bg-white px-5 py-4 sm:px-7"><div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-emerald-700"><FileSpreadsheet className="h-4 w-4" />Bulk product entry</span><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">Excel-style grid</span>{draftSaved && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700">Draft saved</span>}</div><h2 className="mt-1 text-xl font-black text-slate-950 sm:text-2xl">Add products quickly and accurately</h2><p className="mt-1 text-sm text-slate-500">Paste from Excel, choose master data, review issues, then click <b>Save products</b>.</p></div><button type="button" disabled={busy} onClick={() => setBulkOpen(false)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-700 text-white transition hover:bg-emerald-800 disabled:opacity-40"><X className="h-5 w-5" /></button></div></div>
...
function validateRows(rows: ProductRow[]) {
  ...
  rows.forEach((row, index) => {
    ...
    for (const key of ['Purchase Price']) if (!row[key]?.trim() || !Number.isFinite(Number(row[key])) || Number(row[key]) < 0) errors.push({ row: rowNumber, message: `${key} must be a valid non-negative number` })
...
function Cell({ column, row, rowIndex, colIndex, activeCell, setActiveCell, updateCell, renderMasterSelect }: { column: Column; row: ProductRow; rowIndex: number; colIndex: number; activeCell: { row: number; col: number }; setActiveCell: (value: { row: number; col: number }) => void; updateCell: (rowIndex: number, key: string, value: string) => void; renderMasterSelect: (row: ProductRow, rowIndex: number, columnKey: string, options: MasterOption[], placeholder: string) => ReactNode }) {
  const masterMap: Record<string, MasterOption[]> = {}
  const isMaster = ['Category', 'Subcategory', 'Brand', 'Unit'].includes(column.key)
  if (isMaster) {
    return <div className={activeCell.row === rowIndex && activeCell.col === colIndex ? 'rounded-lg ring-2 ring-amber-300' : ''}>{renderMasterSelect(row, rowIndex, column.key, masterMap[column.key] ?? [], `Select ${column.label.toLowerCase()}`)}</div>
  }
  const isActive = activeCell.row === rowIndex && activeCell.col === colIndex
  const isRequiredMissing = Boolean(column.required && !row[column.key]?.trim())
  return <input value={row[column.key] ?? ''} onFocus={() => setActiveCell({ row: rowIndex, col: colIndex })} onChange={event => updateCell(rowIndex, column.key, event.target.value)} placeholder={column.required ? 'Required' : 'Optional'} type={['Purchase Price', 'Sale Price', 'Opening Stock', 'Current Stock', 'Reorder Level'].includes(column.key) ? 'number' : 'text'} min={0} className={`h-10 w-full rounded-lg border px-2.5 text-sm font-semibold outline-none transition ${isRequiredMissing ? 'border-rose-200 bg-rose-50/40' : 'border-slate-200 bg-white'} ${isActive ? 'border-amber-300 bg-amber-50/40 ring-2 ring-amber-200' : 'hover:border-emerald-300 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/10'}`} />
}
