...
export default function DashboardPage() {
  ...
  return <>
    <main className="min-w-0 space-y-5 overflow-x-hidden pb-8">
      ...
      {role === 'admin' && <Link href="/dashboard/cash-bank-collection" className="group block rounded-[26px] border border-emerald-200 bg-gradient-to-r from-emerald-50 via-white to-blue-50 p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-emerald-700">Accounts & Finance · Admin</p><h2 className="mt-1 text-lg font-black text-slate-950">Cash / Bank Collection</h2><p className="mt-1 text-xs text-slate-500">Open today, 7-day or custom collection totals</p></div><span className="rounded-xl bg-white px-4 py-2 text-xs font-black text-emerald-800 ring-1 ring-emerald-200">Open report →</span></div><div className="mt-4 grid grid-cols-3 gap-2 sm:max-w-2xl"><div className="rounded-2xl bg-white p-3 ring-1 ring-slate-100"><p className="text-[10px] font-black uppercase text-slate-400">Cash</p><p className="mt-1 text-lg font-black text-emerald-700">{money(s.cash)}</p></div><div className="rounded-2xl bg-white p-3 ring-1 ring-slate-100"><p className="text-[10px] font-black uppercase text-slate-400">Bank</p><p className="mt-1 text-lg font-black text-blue-700">{money(s.bank)}</p></div><div className="rounded-2xl bg-white p-3 ring-1 ring-slate-100"><p className="text-[10px] font-black uppercase text-slate-400">Cash + Bank</p><p className="mt-1 text-lg font-black text-slate-950">{money(Number(s.cash)+Number(s.bank))}</p></div></div></Link>}
