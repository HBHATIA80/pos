        ...
        <div className="flex flex-1 items-center justify-center px-5 pb-10 pt-2 sm:px-8 lg:px-12 lg:pb-16">
          <div className="w-full max-w-xl">
            ...
            <form onSubmit={handleLogin} className="rounded-[28px] border-2 border-slate-200 bg-white p-5 shadow-xl shadow-slate-300/40 sm:p-7">
              ...
              <div className="mt-5"><div className="mb-2 flex items-center justify-between"><label htmlFor="password" className="text-sm font-bold text-slate-950">Password</label><Link href="/forgot-password" className="text-sm font-black text-[#087f73] hover:text-[#06665d]">Forgot password?</Link></div><div className="relative"><Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-slate-600" /><input id="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" enterKeyHint="done" required value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" className={`${inputClass} pl-11 pr-11`} /><button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-slate-600 hover:bg-slate-100 hover:text-slate-950" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></div>
              <button type="submit" disabled={loading} className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0f8f83] px-5 text-sm font-black text-white shadow-lg shadow-[#0f8f83]/25 transition hover:bg-[#0b776d] disabled:cursor-not-allowed disabled:opacity-70">{loading ? <><Loader2 className="h-5 w-5 animate-spin" /> Signing you in...</> : <>Continue <ArrowRight className="h-4 w-4" /></>}</button>
            </form>
            <div className="mt-6 space-y-3 text-center text-sm font-medium text-slate-700"><p>Opening a shop? <Link href="/signup" className="font-black text-[#087f73] hover:text-[#06665d]">Create Shop Account</Link></p><p>Already a customer? <Link href="/customer-signup" className="font-black text-[#087f73] hover:text-[#06665d]">Create Customer Account</Link></p><a href={supportUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 font-bold text-emerald-800 hover:text-emerald-900"><MessageCircle className="h-4 w-4" /> Questions or suggestions? Contact support</a></div>
          </div>
        </div>
      </section>
    </main>
