...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  if (!start.success || (end && !end.success)) {
  ...
  const { data: invoices, error: invoiceError } = await invoiceBase.order('completed_at', { ascending: true })
  ...
  for (const invoice of invoices ?? []) {
    const date = invoice.completed_at ?? invoice.sold_at ?? invoice.created_at
    if (date < startAt) continue
    if (endExclusive && date >= endExclusive.toISOString()) continue
    entries.push({
      id: invoice.id,
      date,
      type: 'invoice',
  ...
  for (const payment of payments ?? []) {
    if (payment.paid_at < startAt) continue
    if (endExclusive && payment.paid_at >= endExclusive.toISOString()) continue
    entries.push({
      id: payment.id,
      date: payment.paid_at,
      type: 'payment',
      description: payment.payment_method === 'cash' ? 'Cash Payment Received' : 'Bank Payment Received',
  ...
  return NextResponse.json({
    ...
    period: { start_date: rawStart, end_date: rawEnd || null },
