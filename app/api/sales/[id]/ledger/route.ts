...
export async function GET(
  ...
  if (invoiceError) {
    console.error('Ledger invoice lookup error:', invoiceError)
  ...
  if (paymentError) {
    console.error('Ledger payment lookup error:', paymentError)
  ...
  const paymentStatus =
    paidAmount <= 0 ? 'unpaid' : balanceAmount <= 0 ? 'paid' : 'partial'
  ...
  const ledger = [
    ...
    ...activePayments.map((payment) => {
      const credit = Number(payment.amount ?? 0)
