...
export async function GET(request: NextRequest) {
  ...
  if (categoriesResult.error || subcategoriesResult.error || brandsResult.error) return NextResponse.json({ error: 'Unable to load product filters' }, { status: 400 })

  const total = count ?? 0
  return NextResponse.json({
    products: data || [], total, offset, limit,
