...
async function getContext() {
  ...
  const { data: profile } = await supabase.from('profiles').select('id,business_id,is_active').eq('id', user.id).maybeSingle()
...
export async function POST(request: NextRequest) {
  ...
  if (value.party_id) {
    const { data: party, error: partyError } = await supabase.from('parties').select('id').eq('id', value.party_id).eq('business_id', profile.business_id).maybeSingle()
    if (partyError) return NextResponse.json({ error: partyError.message }, { status: 400 })
    if (!party) return NextResponse.json({ error: 'Party not found' }, { status: 404 })
  }

  const { data, error } = await supabase.from('account_vouchers').insert({
