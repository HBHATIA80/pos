...
export async function GET(request: NextRequest) {
  ...
  if (!profile?.is_active || !profile.business_id || !['admin', 'staff'].includes(profile.role)) return NextResponse.json({ error: 'Admin or staff access required.' }, { status: 403 })
  ...
  const from = params.get('from') || null; const to = params.get('to') || null
  const minAmountRaw = params.get('min_amount'); const maxAmountRaw = params.get('max_amount')
  ...
  if (should('sales')) {
    ...
    for (const row of data ?? []) { const party = relation(row.parties); const date = row.sold_at || row.created_at; rows.push({ id: `sale-${row.id}`, type: 'sales', number: row.invoice_no, party: party?.name || 'Walk-in customer', date, amount: Number(row.grand_total) || 0, status: row.status, method: 'Invoice', description: row.notes || 'Sales invoice' }) }
  }
  ...
  if (should('purchases')) {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
    for (const row of data ?? []) { const party = relation(row.party); const date = row.purchased_at || row.created_at; rows.push({ id: `purchase-${row.id}`, type: 'purchases', number: row.invoice_no, party: party?.name || 'Cash purchase', date, amount: Number(row.grand_total) || 0, status: row.status, method: 'Invoice', description: row.notes || 'Purchase invoice' }) }
  }

  if (should('payments') || should('receipts')) {
    const voucherType = should('payments') && should('receipts') ? null : should('payments') ? 'payment' : 'receipt'
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
    for (const row of data ?? []) { const party = relation(row.parties); const rowType = row.voucher_type === 'receipt' ? 'receipts' : 'payments'; rows.push({ id: `voucher-${row.id}`, type: rowType, number: row.voucher_no, party: party?.name || row.account_name || 'General', date: row.paid_at, amount: Number(row.amount) || 0, status: row.status, method: row.payment_method, description: row.notes || row.reference_no || `${row.voucher_type} voucher` }) }
    if (should('receipts')) {
      ...
      for (const row of salePayments ?? []) { const party = relation(row.parties); const invoice = relation(row.sales_invoices); rows.push({ id: `sale-payment-${row.id}`, type: 'receipts', number: row.receipt_no, party: party?.name || 'Walk-in customer', date: row.paid_at, amount: Number(row.amount) || 0, status: row.status, method: row.payment_method, description: row.notes || `Receipt against ${invoice?.invoice_no || 'invoice'}` }) }
    }
  }
