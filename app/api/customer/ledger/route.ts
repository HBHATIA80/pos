...
export async function GET() {
  ...
  // Customer ledger ownership is based on the customer's party record.
  // It must not depend on who created the invoice, because shop staff may
  // create invoices for the same customer.
  let partyId: string | null = profile.party_id ?? null
  let partyName: string | null = null

  if (partyId) {
    const { data: linkedParty, error: linkedPartyError } = await supabase
      .from('parties')
    ...
    if (linkedPartyError) {
      console.error('Customer ledger linked party lookup error:', linkedPartyError)
    } else if (linkedParty?.is_active) {
  ...
  if (!partyId && profile.phone) {
    ...
    if (partyPhoneError) {
      console.error('Customer ledger party phone lookup error:', partyPhoneError)
    } else if (partyByPhone) {
  ...
  if (partyId) {
    ...
    // Legacy compatibility only. New staff-created/customer-created sales
    // must be linked to the customer's party.
    invoiceQuery = invoiceQuery.eq('created_by', user.id)
  }

  const { data: invoices, error: invoiceError } = await invoiceQuery.order('completed_at', {
    ascending: true,
    nullsFirst: false,
  })
  ...
  const purchaseTotal = invoiceRows.reduce((sum, invoice) => sum + Number(invoice.grand_total ?? 0), 0)
