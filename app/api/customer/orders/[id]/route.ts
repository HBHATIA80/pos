...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  const { data: invoice, error } = await supabase
    .from('sales_invoices')
    .select(`id,invoice_no,status,order_channel,order_status,grand_total,subtotal,discount_amount,notes,created_at,sold_at,completed_at,created_by,sales_invoice_items(id,product_id,product_name,sku,unit_name,quantity,unit_price,discount_amount,line_total),sales_order_events(id,status,note,acted_by,created_at)`)
    .eq('id', id.data)
    .eq('business_id', profile.business_id)
    .eq('order_channel', 'customer_portal')
    .maybeSingle()

  if (error) return NextResponse.json({ error: error.message || 'Unable to load order' }, { status: 400 })
  if (!invoice) return NextResponse.json({ error: 'Order not found' }, { status: 404 })

  const canView = profile.role === 'admin' || profile.role === 'staff' || (profile.role === 'user' && invoice.created_by === user.id)
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  const { supabase, user, profile } = await getContext()
  if (!user || !profile?.is_active || !profile.business_id) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if (profile.role !== 'admin' && profile.role !== 'staff') return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  if (!id.success || !status.success || !note.success) return NextResponse.json({ error: 'Invalid order action' }, { status: 400 })
  ...
  if (error) return NextResponse.json({ error: error.message || 'Unable to update order' }, { status: 400 })
