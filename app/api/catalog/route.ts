...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from(table).select('*').eq('business_id', businessId).order('name'); if (error) return NextResponse.json({ error: error.message }, { status: 400 }); return NextResponse.json({ [entity.data]: data ?? [] })
}
...
export async function PATCH(request: Request) {
  ...
  if (entity.data === 'products' && 'marketplace_enabled' in payload) {
    const { data: enabled, error } = await supabase.rpc('set_product_marketplace', { p_product_id: id.data, p_enabled: Boolean(payload.marketplace_enabled) })
    if (error) return NextResponse.json({ error: error.message || 'Unable to update marketplace listing' }, { status: 400 })
    return NextResponse.json({ marketplace_enabled: Boolean(enabled) })
  }
  ...
  const updates: Record<string, unknown> = {}; for (const key of allowed) if (key in payload) updates[key] = payload[key] === '' ? null : payload[key]
  if (!Object.keys(updates).length) return NextResponse.json({ error: 'Nothing to update' }, { status: 400 })
  const table = entity.data === 'products' ? 'products' : entity.data === 'categories' ? 'catalog_categories' : entity.data === 'subcategories' ? 'catalog_subcategories' : entity.data === 'brands' ? 'catalog_brands' : 'catalog_units'
  const { data, error } = await supabase.from(table).update(updates).eq('id', id.data).eq('business_id', profile.business_id).select('*').single(); if (error) return NextResponse.json({ error: error.message }, { status: 400 }); return NextResponse.json({ item: data })
}
