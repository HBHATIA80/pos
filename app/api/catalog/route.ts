...
export async function GET(request: Request) {
  ...
  const entity = entitySchema.safeParse(new URL(request.url).searchParams.get('entity') ?? 'products')
  if (!entity.success) return NextResponse.json({ error: 'Invalid catalog entity' }, { status: 400 })

  // Customer-facing users can only read the product catalog.
  if (profile.role === 'user' && entity.data !== 'products') {
    return NextResponse.json({ error: 'Catalog master access is restricted.' }, { status: 403 })
  }
  ...
  if (entity.data === 'products') {
    const { data, error } = await supabase
      .from('products')
      .select('id, sku, barcode, name, description, category_id, subcategory_id, brand_id, unit_id, purchase_price, sale_price, opening_stock, current_stock, reorder_level, is_active, catalog_categories(id,name), catalog_subcategories(id,name), catalog_brands(id,name), catalog_units(id,name,short_name,decimal_places)')
      .eq('business_id', businessId)
      .order('name')

    if (error) return NextResponse.json({ error: error.message }, { status: 400 })

    // Never expose internal purchase cost to customer-facing users.
    const products = profile.role === 'user'
      ? (data ?? []).map(({ purchase_price: _purchasePrice, ...product }) => product)
      : (data ?? [])
  ...
  const table = entity.data === 'categories'
    ? 'catalog_categories'
    : entity.data === 'subcategories'
...
export async function POST(request: Request) {
  ...
  if (profile.role !== 'admin' && profile.role !== 'staff') {
    return NextResponse.json({ error: 'Catalog management is restricted.' }, { status: 403 })
  }
  ...
  if (entity.data === 'products') {
    ...
    row = {
      ...
      sku: parsed.data.sku,
      barcode: cleanOptional(parsed.data.barcode),
      ...
      current_stock: parsed.data.opening_stock,
    ...
    row = {
      ...
      is_active: parsed.data.is_active ?? true,
      created_by: user.id,
      ...(entity.data === 'subcategories' ? { category_id: body?.data?.category_id } : {}),
    }
    if (entity.data === 'subcategories' && !z.string().uuid().safeParse(row.category_id).success) {
      return NextResponse.json({ error: 'Category is required for a subcategory' }, { status: 400 })
    }
  }

  const table = entity.data === 'products'
    ? 'products'
...
export async function PATCH(request: Request) {
  ...
  const payload = body?.data ?? {}
  const allowed = entity.data === 'products'
    ? ['sku','barcode','name','description','category_id','subcategory_id','brand_id','unit_id','purchase_price','sale_price','reorder_level','is_active']
    : entity.data === 'units'
      ? ['name','short_name','decimal_places','is_active']
      : entity.data === 'subcategories'
        ? ['name','code','description','category_id','is_active']
