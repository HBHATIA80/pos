...
async function getContext() {
  ...
  const { data: profile } = await supabase.from('profiles').select('business_id,is_active,role').eq('id', user.id).maybeSingle()
...
export async function GET(request: Request) {
  ...
  if (q) {
    ...
    productQuery = productQuery.or(`name.ilike.%${escaped}%,sku.ilike.%${escaped}%,barcode.ilike.%${escaped}%`)
  }

  const [{ data: products, error: productError }, { data: movements, error: movementError }] = await Promise.all([
    productQuery,
    supabase.from('stock_movements').select('id,product_id,movement_type,quantity,reference_type,reference_id,notes,created_at,products(name,sku)').eq('business_id', profile.business_id).order('created_at', { ascending: false }).limit(150),
  ])
  ...
  return NextResponse.json({ products: products ?? [], movements: movements ?? [], query: q, limit })
}
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc('adjust_stock', { payload: { product_id: parsed.data.product_id, direction: parsed.data.direction, quantity: parsed.data.quantity, notes: parsed.data.notes || null } })
