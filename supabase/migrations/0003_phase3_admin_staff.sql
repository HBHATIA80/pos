...
-- Phase 2's signup trigger created a new business for every auth user. Phase 3
-- extends it so an admin-created staff/user account can carry the existing
-- business_id in Auth metadata without creating another business.
...
begin
  ...
  v_existing_business_id := nullif(new.raw_user_meta_data ->> 'business_id', '')::uuid;

  if v_existing_business_id is not null then
    if not exists (select 1 from public.businesses where id = v_existing_business_id and status = 'active') then
    ...
    values (new.id, v_existing_business_id, v_role, v_full_name, new.phone);
...
comment on function public.handle_new_user() is 'Creates the first business/admin or attaches an admin-created staff/user to an existing business.';
