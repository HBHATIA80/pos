-- Phase 13: selected-shop customer catalog and ordering.
-- Customer order RPC now accepts an explicitly selected shop, but only after
-- validating an active customer_business_memberships row for the authenticated user.
-- This preserves multi-shop isolation while allowing one customer account to order
-- from any connected shop.
...
declare
  ...
  v_requested_business_id uuid := nullif(payload->>'business_id','')::uuid;
...
begin
  ...
  if v_is_order then
    if v_requested_business_id is null then
      raise exception 'Shop selection is required';
    end if;

    select m.business_id, m.party_id
      into v_business_id, v_party_id
    from public.customer_business_memberships m
    where m.user_id = v_user_id
      and m.business_id = v_requested_business_id
      and m.is_active = true
    limit 1;

    if v_business_id is null or v_party_id is null then
  ...
  for v_item in select * from jsonb_array_elements(payload->'items') loop
    ...
    values
      (v_invoice.id, v_product.id, v_product.sku, v_product.name,
