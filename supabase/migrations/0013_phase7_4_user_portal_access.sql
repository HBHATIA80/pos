-- Phase 7.4: user portal access
-- Ordinary users are customer-facing accounts: they may browse active products
-- and place orders, but must not access business administration or sales management.
...
create policy "catalog managers can view categories"
...
create policy "catalog managers can view subcategories"
on public.catalog_subcategories for select
to authenticated
using (business_id = public.current_business_id() and public.has_permission('catalog.manage'));

create policy "catalog managers can view brands"
on public.catalog_brands for select
to authenticated
using (business_id = public.current_business_id() and public.has_permission('catalog.manage'));

create policy "catalog managers can view units"
...
drop policy if exists "business members can view sales invoices" on public.sales_invoices;
drop policy if exists "sales managers can insert sales invoices" on public.sales_invoices;
drop policy if exists "business members can view sales invoice items" on public.sales_invoice_items;
drop policy if exists "sales managers can insert sales invoice items" on public.sales_invoice_items;
...
using (
  ...
  and (
    ...
    or (public.has_permission('orders.place') and created_by = auth.uid())
  )
);

create policy "sales or order members can insert invoices"
on public.sales_invoices for insert
to authenticated
with check (
  business_id = public.current_business_id()
  and (
    public.has_permission('sales.manage')
    or (public.has_permission('orders.place') and status = 'draft' and created_by = auth.uid())
  )
);
  ...
  exists (
    ...
    where i.id = invoice_id
      ...
      and (
        public.has_permission('sales.view')
        or public.has_permission('sales.manage')
        or (public.has_permission('orders.place') and i.created_by = auth.uid())
      )
  )
);

create policy "sales or order members can insert invoice items"
...
is 'Creates business sales invoices for managers and draft customer orders for users; customer orders always use the current product sale price.';
