...
create table public.sales_invoice_items (
  ...
  constraint sales_invoice_items_total_check check (line_total = (quantity * unit_price) - discount_amount)
);
...
begin
  ...
  if v_party_id is not null then
    ...
    where id = v_party_id and business_id = v_business_id and is_active = true;
  ...
  if jsonb_typeof(coalesce(payload->'items','null')) <> 'array' or jsonb_array_length(payload->'items') = 0 then
  ...
  for v_item in select * from jsonb_array_elements(payload->'items') loop
    ...
    if coalesce((v_item->>'quantity')::numeric, 0) <= 0 then raise exception 'Quantity must be greater than zero'; end if;
    if coalesce((v_item->>'unit_price')::numeric, -1) < 0 then raise exception 'Unit price cannot be negative'; end if;
    if coalesce((v_item->>'discount_amount')::numeric, 0) < 0 then raise exception 'Discount cannot be negative'; end if;
    ...
    if ((v_item->>'line_total')::numeric) < 0 then raise exception 'Line total cannot be negative'; end if;
...
begin
  ...
  where id = invoice_id and business_id = public.current_business_id();
  ...
  update public.sales_invoices
  set status='completed', sold_at=coalesce(sold_at, now()), completed_at=now()
  where id=v_invoice.id
  returning * into v_invoice;
  return v_invoice;
end;
...
with check (business_id = public.current_business_id() and public.has_permission('sales.manage'));
