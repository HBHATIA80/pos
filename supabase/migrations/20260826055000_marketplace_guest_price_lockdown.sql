...
as $$
  select
    ...
    p.sku,
    p.barcode,
    case when auth.uid() is null then null else p.sale_price end,
    p.image_url,
    p.category_id,
    p.subcategory_id,
    p.brand_id,
...
revoke execute on function public.marketplace_products(text, integer, integer) from anon;
revoke execute on function public.customer_marketplace_products(text, integer, integer) from anon;
