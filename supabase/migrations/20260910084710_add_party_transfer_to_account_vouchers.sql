alter table public.account_vouchers add column if not exists destination_party_id uuid references public.parties(id) on delete restrict;
create index if not exists account_vouchers_destination_party_idx on public.account_vouchers (business_id, destination_party_id, paid_at) where destination_party_id is not null and status = 'active';
